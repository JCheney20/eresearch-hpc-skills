// Imported from https://github.com/swcarpentry/shell-novice/blob/22c5a874725bd3048eb7cfceeafd0db3a5e49a2f/episodes/05-loop.md
...
When the shell sees the keyword `for`, it knows to repeat a command (or group of commands) once for each item in a list. Each time the loop runs (called an iteration), an item in the list is assigned in sequence to the #strong[variable];, and the commands inside the loop are executed, before moving on to the next item in the list. Inside the loop, we call for the variable's value by putting `$` in front of it. The `$` tells the shell interpreter to treat the variable as a variable name and substitute its value in its place, rather than treat it as text or an external command.
...
If the #emph[shell] prints `>` or `$` then it expects you to type something, and the symbol is a prompt.

If #emph[you] type `>` or `$` yourself, it is an instruction from you that the shell should redirect output or get the value of a variable.
...
it would work exactly the same way. #emph[Don't do this.] Programs are only useful if people can understand them, so meaningless names (like `x`) or misleading names (like `temperature`) increase the odds that the program won't do what its readers think it does.
...
Since the `cp` command does not normally produce any output, it's hard to check that the loop is working correctly. However, we learned earlier how to print strings using `echo`, and we can modify the loop to use `echo` to print our commands without actually executing them. As such we can check what commands #emph[would be] run in the unmodified loop.
...
#box(image("fig/shell_script_for_loop_flow_chart.svg"))

== Nelle's Pipeline: Processing Files
<nelles-pipeline-processing-files>
...
The reduced sample dataset used here has 17 files. Thus, 17 times 5 seconds, tells her that her script will take about 85 seconds to run. The full dataset has 1520 files, which would take about 2 hours to run. As a final check, she opens another terminal window, goes into `north-pacific-gyre`, and uses `cat stats-NENE01729B.txt` to examine one of the output files. It looks good, so she decides to catch up on social media posts while waiting.
...
== Doing a Dry Run
<doing-a-dry-run>
A loop is a way to do many things at once --- or to make many mistakes at once if it does the wrong thing. One way to check what a loop #emph[would] do is to `echo` the commands it would run instead of actually running them.
...
== Solution
<solution-6>
The second version is the one we want to run. This prints to screen everything enclosed in the quote marks, expanding the loop variable name because we have prefixed it with a dollar sign. It also #emph[does not] modify nor create the file `all.pdb`, as the `>>` is treated literally as part of a string rather than as a redirection instruction.
...
== Nested Loops
<nested-loops>
Suppose we want to set up a directory structure to organize some experiments measuring reaction rate constants with different compounds #emph[and] different temperatures. What would be the result of the following code:
