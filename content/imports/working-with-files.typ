// Imported from https://github.com/swcarpentry/shell-novice/blob/22c5a874725bd3048eb7cfceeafd0db3a5e49a2f/episodes/03-create.md
...
#block[
One could use `ls -R` to list a directory hierarchy, but rather than introducing it in `cp` context, `find` can be used instead and is explained more deeply in the `find` episode.
...
If you need to refer to names of files or directories that have spaces or other special characters, you should surround the name in single #link("https://www.gnu.org/software/bash/manual/html_node/Quoting.html")[quotes] (`''`).
...
== Which Editor?
<which-editor>
When we say, '`nano` is a text editor' we really do mean 'text'. It can only work with plain character data, not tables, images, or any other human-friendly media. We use it in examples because it is one of the least complex text editors. However, because of this trait, it may not be powerful enough or flexible enough for the work you need to do after this workshop. On Unix systems (such as Linux and macOS), many programmers use #link("https://www.gnu.org/software/emacs/")[Emacs] or #link("https://www.vim.org/")[Vim] (both of which require more time to learn), or a graphical editor such as #link("https://gedit-text-editor.org/")[Gedit] or #link("https://code.visualstudio.com/")[VScode];. On Windows, you may wish to use #link("https://notepad-plus-plus.org/")[Notepad++];. Windows also has a built-in editor called `notepad` that can be run from the command line in the same way as `nano` for the purposes of this lesson.
...
#box(image("fig/nano-screenshot.png"))
...
Naming a PNG image of a whale as `whale.mp3` doesn't somehow magically turn it into a recording of whale song, though it #emph[might] cause the operating system to associate the file with a music player program. In this case, if someone double-clicked `whale.mp3` in a file explorer program, the music player will automatically (and erroneously) attempt to open the `whale.mp3` file.
...
The `-r` or `--recursive` option for `cp` applies the command not just to the specified directory, but to all its contents.
...
<destination-can-be-new-name-or-directory>
When we use two arguments with `mv` or `cp`, the second argument can be the location and new name we want our file to have. Alternatively, if we provide the path to an existing directory as the second argument, `mv`/`cp` will interpret that to mean that we want the file copied/moved to that directory without changing its name.
...
`rm` can remove a directory #emph[and all its contents] if we use the recursive option `-r`, and it will do so #emph[without any confirmation prompts];:

```bash
$ rm -r thesis
```

Given that there is no way to retrieve files deleted using the shell, `rm -r` #emph[should be used with great caution] (you might consider adding the interactive option `rm -r -i`).
...
We have seen how `cp` behaves when given two arguments. The second argument can be a filename (what to name the copy) or an existing directory (where to put the copy).

`cp` behaves slightly differently when given three or more arguments. Let's try giving `cp` three arguments. In the example below, what does `cp` do when given several filenames and a directory name?
...
== Solution
<solution-5>
When `cp` is given three or more arguments, the last argument must be a directory. `cp` copies the other files specified into that directory. If the last argument is a file name, you get an error:
...
== Wildcards
<wildcards>
`*` is a #strong[wildcard];, which represents zero or more other characters. Let's consider the `shell-lesson-data/exercise-data/alkanes` directory: `*.pdb` represents `ethane.pdb`, `propane.pdb`, and every file that ends with '.pdb'. On the other hand, `p*.pdb` only represents `pentane.pdb` and `propane.pdb`, because the 'p' at the front can only represent filenames that begin with the letter 'p'.
...
When the shell sees a wildcard, it expands the wildcard to create a list of matching filenames #emph[before] running the preceding command. As an exception, if a wildcard expression does not match any file, Bash will pass the expression as an argument to the command as it is. For example, typing `ls *.pdf` in the `alkanes` directory (which contains only files with names ending with `.pdb`) results in an error message that there is no file called `*.pdf`. However, generally commands like `wc` and `ls` see the lists of file names matching these expressions, but not the wildcards themselves. It is the shell, not the other programs, that expands the wildcards.
