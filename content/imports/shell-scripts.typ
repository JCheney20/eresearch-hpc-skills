// Imported from https://github.com/swcarpentry/shell-novice/blob/22c5a874725bd3048eb7cfceeafd0db3a5e49a2f/episodes/06-script.md
...
This is a variation on the pipe we constructed earlier, which selects lines 11-15 of the file `octane.pdb`. Remember, we are #emph[not] running it as a command just yet; we are only incorporating the commands in a file.
...
This works, but it may take the next person who reads `middle.sh` a moment to figure out what it does. We can improve our script by adding some #strong[comments] at the top:
...
the last command in the file is the `history` command itself, i.e., the shell has added `history` to the command log before actually running it. In fact, the shell #emph[always] adds commands to the log before running them. Why do you think it does this?
...
<nelles-pipeline-creating-a-script>
...
The first part of the pipeline, `wc -l $1/*.$2 | sort -n`, counts the lines in each file and sorts them numerically (largest last). When there's more than one file, `wc` also outputs a final summary line, giving the total number of lines across #emph[all] files. We use `tail -n 2 | head -n 1` to throw away this last line.
...
Script 3 would print all the arguments to the script (i.e.~all the `.pdb` files), followed by `.pdb`. `$@` refers to #emph[all] the arguments given to a shell script.
