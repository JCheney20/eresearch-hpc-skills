// Imported from https://github.com/swcarpentry/git-novice/blob/967bc0b38826039f6554845248c8c294ebff1f56/episodes/07-github.md
...
Systems like Git allow us to move work between any two repositories. In practice, though, it's easiest to use one copy as a central hub, and to keep it on the web rather than on someone's laptop. Most programmers use hosting services like #link("https://github.com")[GitHub];, #link("https://bitbucket.org")[Bitbucket] or #link("https://gitlab.com/")[GitLab] to hold those main copies; we'll explore the pros and cons of this in a #link("13-hosting.md")[later episode];.

Let's start by sharing the changes we've made to our current project with the world. To this end we are going to create a #emph[remote] repository that will be linked to our #emph[local] repository.

== 1. Create a remote repository
<create-a-remote-repository>
Log in to #link("https://github.com")[GitHub];, then click on the icon in the top right corner to create a new repository called `recipes`:

#box(image("fig/github-create-repo-01.png"))
...
#box(image("fig/github-create-repo-02.png"))

As soon as the repository is created, GitHub displays a page with a URL and some information on how to configure your local repository:

#box(image("fig/github-create-repo-03.png"))
...
If you remember back to the earlier #link("04-changes.md")[episode] where we added and committed our earlier work on `guacamole.md`, we had a diagram of the local repository which looked like this:

#box(image("fig/git-staging-area.svg"))

Now that we have two repositories, we need a diagram like this:

#box(image("fig/git-freshly-made-github-repo.svg"))
...
Now we connect the two repositories. We do this by making the GitHub repository a #link("../learners/reference.md#remote")[remote] for the local repository. The home page of the repository on GitHub includes the URL string we need to identify it:

#box(image("fig/github-change-repo-string.png"))

Click on the 'SSH' link to change the #link("../learners/reference.md#protocol")[protocol] from HTTPS to SSH.
...
#box(image("fig/github-find-repo-string.png"))
...
<resolving-ssh-key-conflicts-custom-names-and-paths>
If you need to create an SSH key pair with a custom name or store it in a non-default location (e.g., because a default-named key like id\_ed25519 already exists), Git may not automatically use it when pushing or pulling from GitHub.
...
in the terminal, before you run `git push`. Despite the name, #link("https://git-scm.com/docs/gitcredentials#_requesting_credentials")[Git uses `SSH_ASKPASS` for all credential entry];, so you may want to unset `SSH_ASKPASS` whether you are using Git via SSH or https.
...
#box(image("fig/github-repo-after-first-push.svg"))
...
<uploading-files-directly-in-github-browser>
Github also allows you to skip the command line and upload files directly to your repository without having to leave the browser. There are two options. First you can click the "Upload files" button in the toolbar at the top of the file tree. Or, you can drag and drop files from your desktop onto the file tree. You can read more about this #link("https://help.github.com/articles/adding-a-file-to-a-repository/")[on this GitHub page];.

]
#block[
== GitHub Timestamp
<github-timestamp>
Create a remote repository on GitHub. Push the contents of your local repository to the remote. Make changes to your local repository and push these changes. Go to the repo you just created on GitHub and check the #link("../learners/reference.md#timestamp")[timestamps] of the files. How does GitHub record times, and why?
