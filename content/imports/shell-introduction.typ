// Imported from https://github.com/swcarpentry/shell-novice/blob/22c5a874725bd3048eb7cfceeafd0db3a5e49a2f/episodes/01-intro.md
...
=== What is the Shell?
<what-is-the-shell>
Humans and computers commonly interact in many different ways, such as through a keyboard and mouse, touch screen interfaces, or using speech recognition systems. The most widely used way to interact with personal computers is called a #strong[graphical user interface] (GUI). With a GUI, we give instructions by clicking a mouse and using menu-driven interactions.

While the visual aid of a GUI makes it intuitive to learn, this way of interacting scales very poorly for some tasks.

A #strong[command-line interface] (CLI) allows users to interact with a computer by reading and writing text. It excels at making repetitive tasks automatic and fast.

A #strong[shell] is a particular program that lets you type commands. In this workshop, we will be using "Bash" which is the most popular Unix shell. Bash is often the default shell on Unix and in Unix-like tools for Windows.
...
=== Let's get started.
<lets-get-started.>
When the shell is first opened, you are presented with a #strong[prompt];, indicating that the shell is waiting for input.

```bash
$
```

The shell typically uses `$` as the prompt, but may use a different symbol. In the examples for this lesson, we'll show the prompt as `$`. Most importantly, #emph[do not type the prompt] when typing commands. Only type the command that follows the prompt. This rule applies both in these lessons and in lessons from other sources. Also note that after you type a command, you have to press the Enter key to execute it.

The prompt is followed by a #strong[text cursor];, a character that indicates the position where your typing will appear. The cursor is usually a flashing or solid block, but it can also be an underscore or a pipe. You may have seen it in a text editor program, for example.
...
<nelles-pipeline-a-typical-problem>
Nelle Nemo, a marine biologist, has just returned from a six-month survey of the #link("https://en.wikipedia.org/wiki/North_Pacific_Gyre")[North Pacific Gyre];, where she has been sampling gelatinous marine life in the #link("https://en.wikipedia.org/wiki/Great_Pacific_Garbage_Patch")[Great Pacific Garbage Patch];. She has 1520 samples that she's run through an assay machine to measure the relative abundance of 300 proteins. She needs to run these 1520 files through an imaginary program called `goostats.sh`. In addition to this huge task, she has to write up results by the end of the month, so her paper can appear in a special issue of #emph[Aquatic Goo Letters];.
