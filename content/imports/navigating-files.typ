// Imported from https://github.com/swcarpentry/shell-novice/blob/22c5a874725bd3048eb7cfceeafd0db3a5e49a2f/episodes/02-filedir.md
...
#block[
Introducing and navigating the filesystem in the shell (covered in #link("02-filedir.md")[Navigating Files and Directories] section) can be confusing. You may have both terminal and GUI file explorer open side by side so learners can see the content and file structure while they're using terminal to navigate the system.

]
The part of the operating system responsible for managing files and directories is called the #strong[file system];. It organizes our data into files, which hold information, and directories (also called 'folders'), which hold files or other directories.

Several commands are frequently used to create, inspect, rename, and delete files and directories. To start exploring them, we'll go to our open shell window.

First, let's find out where we are by running a command called `pwd` (which stands for 'print working directory'). Directories are like #emph[places] --- at any time while we are using the shell, we are in exactly one place called our #strong[current working directory];. Commands mostly read and write files in the current working directory, i.e.~'here', so knowing where you are before running a command is important. `pwd` shows you where you are:
...
Here, the computer's response is `/Users/nelle`, which is Nelle's #strong[home directory];:
...
We will also assume that your `pwd` command returns your user's home directory. If `pwd` returns something different, you may need to navigate there using `cd` or some commands in this lesson will not work as written. See #link(<exploring-other-directories>)[Exploring Other Directories] for more details on the `cd` command.
...
#box(image("fig/filesystem.svg"))

The filesystem looks like an upside down tree. The topmost directory is the #strong[root directory] that holds everything else. We refer to it using a slash character, `/`, on its own; this character is the leading slash in `/Users/nelle`.
...
== Slashes
<slashes>
Notice that there are two meanings for the `/` character. When it appears at the front of a file or directory name, it refers to the root directory. When it appears #emph[inside] a path, it's just a separator.

]
Underneath `/Users`, we find one directory for each user with an account on Nelle's machine, her colleagues #emph[imhotep] and #emph[larry];.

#box(image("fig/home-directories.svg"))

The user #emph[imhotep];'s files are stored in `/Users/imhotep`, user #emph[larry];'s in `/Users/larry`, and Nelle's in `/Users/nelle`. Nelle is the user in our examples here; therefore, we get `/Users/nelle` as our home directory. Typically, when you open a new command prompt, you will be in your home directory to start.
...
`ls` prints the names of the files and directories in the current directory. We can make its output more comprehensible by using the `-F` #strong[option] which tells `ls` to classify the output by adding a marker to file and directory names to indicate what they are:
...
Here, we can see that the home directory contains only #strong[sub-directories];. Any names in the output that don't have a classification symbol are #strong[files] in the current working directory.

#block[
== Clearing your terminal
<clearing-your-terminal>
If your screen gets too cluttered, you can clear your terminal using the `clear -x` command. You can still access previous commands using ↑ and ↓ to move line-by-line, or by scrolling in your terminal.

For some Bash configurations, `clear` may also clear the scrollbuffer and you will not be able to scroll back after using `clear`; using `clear -x` prevents this. An alternative to `clear -x` is the keyboard shortcut `Control-L`.

]
=== Getting help
<getting-help>
`ls` has lots of other #strong[options];. There are two common ways to find out how to use a command and what options it accepts --- #strong[depending on your environment, you might find that only one of these ways works:]

+ We can pass a `--help` option to any command (available on Linux and Git Bash), for example:
...
+ We can read its manual with `man` (available on Linux and macOS):
]
...
<help-for-built-in-commands>
Some commands are built in to the Bash shell, rather than existing as separate programs on the filesystem. One example is the `cd` (change directory) command. If you get a message like `No manual entry for cd`, try `help cd` instead. The `help` command is how you get usage information for #link("https://www.gnu.org/software/bash/manual/html_node/Bash-Builtins.html")[Bash built-ins];.
...
To #strong[quit] the `man` pages, press q.
...
GNU provides links to its #link("https://www.gnu.org/manual/manual.html")[manuals] including the #link("https://www.gnu.org/software/coreutils/manual/coreutils.html")[core GNU utilities];, which covers many commands introduced within this lesson.
...
== Solution
<solution>
The `-l` option makes `ls` use a #strong[l];ong listing format, showing not only the file/directory names but also additional information, such as the file size and the time of its last modification. If you use both the `-h` option and the `-l` option, this makes the file size '#strong[h];uman readable', i.e.~displaying something like `5.3K` instead of `5369`.
...
<exploring-other-directories>
Not only can we use `ls` on the current working directory, but we can use it to list the contents of a different directory. Let's take a look at our `Desktop` directory by running `ls -F Desktop`, i.e., the command `ls` with the `-F` #strong[option] and the #link("https://swcarpentry.github.io/shell-novice/reference.html#argument")[#strong[argument];] `Desktop`. The argument `Desktop` tells `ls` that we want a listing of something other than our current working directory:
...
`..` is a special directory name meaning "the directory containing this one", or more succinctly, the #strong[parent] of the current directory. Sure enough, if we run `pwd` after running `cd ..`, we're back in `/Users/nelle/Desktop/shell-lesson-data`:
...
So far, when specifying directory names, or even a directory path (as above), we have been using #strong[relative paths];. When you use a relative path with a command like `ls` or `cd`, it tries to find that location from where we are, rather than from the root of the file system.

However, it is possible to specify the #strong[absolute path] to a directory by including its entire path from the root directory, which is indicated by a leading slash. The leading `/` tells the computer to follow the path from the root of the file system, so it always refers to exactly one directory, no matter where we are when we run the command.
...
== Two More Shortcuts
<two-more-shortcuts>
The shell interprets a tilde (`~`) character at the start of a path to mean "the current user's home directory". For example, if Nelle's home directory is `/Users/nelle`, then `~/data` is equivalent to `/Users/nelle/data`. This only works if it is the first character in the path; `here/there/~/elsewhere` is #emph[not] `here/there/Users/nelle/elsewhere`.

Another shortcut is the `-` (dash) character. `cd` will translate `-` into #emph[the previous directory I was in];, which is faster than having to remember, then type, the full path. This is a #emph[very] efficient way of moving #emph[back and forth between two directories] -- i.e.~if you execute `cd -` twice, you end up back in the starting directory.

The difference between `cd ..` and `cd -` is that the former brings you #emph[up];, while the latter brings you #emph[back];.
...
#box(image("fig/filesystem-challenge.svg"))

#block[
== Solution
<solution-3>
+ No: there #emph[is] a directory `backup` in `/Users`.
...
#box(image("fig/shell_command_syntax.svg"))

`ls` is the #strong[command];, with an #strong[option] `-F` and an #strong[argument] `/`. We've already encountered options which either start with a single dash (`-`), known as #strong[short options];, or two dashes (`--`), known as #strong[long options];. #link("https://swcarpentry.github.io/shell-novice/reference.html#option")[Options] change the behavior of a command and #link("https://swcarpentry.github.io/shell-novice/reference.html#argument")[Arguments] tell the command what to operate on (e.g.~files and directories). Sometimes options and arguments are referred to as #strong[parameters];. A command can be called with more than one option and more than one argument, but a command doesn't always require an argument or an option.

You might sometimes see options being referred to as #strong[switches] or #strong[flags];, especially for options that take no argument. In this lesson we will stick with using the term #emph[option];.
...
Note that the sizes returned by `ls -s` are in #emph[blocks];. As these are defined differently for different operating systems, you may not obtain the same figures as in the example.
...
<nelles-pipeline-organizing-files>
...
This command is a lot to type, but she can let the shell do most of the work through what is called #strong[tab completion];. If she types:
...
This is called #strong[tab completion];, and we will see it in many other tools as we go on.
