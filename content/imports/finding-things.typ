// Imported from https://github.com/swcarpentry/shell-novice/blob/22c5a874725bd3048eb7cfceeafd0db3a5e49a2f/episodes/07-find.md
...
`grep` finds and prints lines in files that match a pattern. For our examples, we will use a file that contains three haiku taken from a #link("https://web.archive.org/web/19991201042211/http://salon.com/21st/chal/1998/01/26chal.html")[1998 competition] in #emph[Salon] magazine (Credit to authors Bill Torcaso, Howard Korder, and Margaret Segall, respectively. See Haiku Error Messages archived #link("https://web.archive.org/web/20000310061355/http://www.salon.com/21st/chal/1998/02/10chal2.html")[Page 1] and #link("https://web.archive.org/web/20000229135138/http://www.salon.com/21st/chal/1998/02/10chal3.html")[Page 2] .). For this set of examples, we're going to be working in the writing subdirectory:
...
Similar to how the `-r` option applies `cp` to an entire directory's contents, we can add `-r` (recursive) to our `grep` command to search for a pattern through all the files in a directory and its subdirectories.
...
== Wildcards
<wildcards>
`grep`'s real power doesn't come from its options, though; it comes from the fact that patterns can include wildcards. (The technical name for these is #strong[regular expressions];, which is what the 're' in 'grep' stands for.) Regular expressions are both complex and powerful; if you want to do complex searches, please look at the lesson on #link("https://librarycarpentry.org/lc-data-intro/01-regular-expressions.html")[our website];. As a taster, we can find lines that have an 'o' in the second position like this:
...
Hint: use `man grep` to remind ourselves how to find text in a directory recursively (meaning include all subdirectories in the search) and `man cut` to select more than one field in a line.
...
== Little Women
<little-women>
You and your friend, having just finished reading #emph[Little Women] by Louisa May Alcott, are in an argument. Of the four sisters in the book, Jo, Meg, Beth, and Amy, your friend thinks that Jo was the most mentioned. You, however, are certain it was Amy. Luckily, you have a file `LittleWomen.txt` containing the full text of the novel (`shell-lesson-data/exercise-data/writing/LittleWomen.txt`). Using a `for` loop, how would you tabulate the number of times each of the four sisters is mentioned?
...
We expected it to find all the text files, but it only prints out `./numbers.txt`. The problem is that the shell expands wildcard characters like `*` #emph[before] commands run. Since `*.txt` in the current directory expands to `./numbers.txt`, the command we actually ran was:
...
Option 2 also works in this instance because the shell tries to expand `*.dat` but there are no `*.dat` files in the current directory, so the wildcard expression gets passed to `find`. We first encountered this in #link("03-create.md")[episode 3];.
...
The Unix shell is older than most of the people who use it. It has survived so long because it is one of the most productive programming environments ever created --- maybe even #emph[the] most productive. Its syntax may be cryptic, but people who have mastered it can experiment with different commands interactively, then use what they have learned to automate their work. Graphical user interfaces may be easier to use at first, but once learned, the productivity in the shell is unbeatable. And as Alfred North Whitehead wrote in 1911, 'Civilization advances by extending the number of important operations which we can perform without thinking about them.'
