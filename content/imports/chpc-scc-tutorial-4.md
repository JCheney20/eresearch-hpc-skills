...
5.  [Continuous Integration Using CircleCI](#continuous-integration-using-circleci)
    ...
    5.  [How to use Ansible and GitHub Actions to run HPL](#how-to-use-ansible-and-github-actions-to-run-hpl)
...
8.  [OpenMX Application Benchmark](#openmx-application-benchmark)
    1.  [Pre-Requisite](#pre-requisite)
    2.  [Setup](#setup)
        1.  [Modify makefile](#modify-makefile)
            1.  [A. Intel (easy difficulty)](#a-intel-easy-difficulty)
            2.  [B. Mix IntelMKL + openMPI + GCC (medium difficulty)](#b-mix-intelmkl--openmpi--gcc-medium-difficulty)
            3.  [C. Own high performace libraries (complex difficulty)](#c-own-high-performace-libraries-complex-difficulty)
                1.  [Build FFTW](#build-fftw)
                2.  [Build OpenBLAS](#build-openblas)
                3.  [ScaLAPACK (Scalable Linear Algebra PACKage)](#scalapackscalable-linear-algebra-package)
                4.  [Setup makefile](#setup-makefile)
    3.  [Tasks](#tasks)
        1.  [Optimization](#optimization)
9.  [OpenFOAM Application Benchmark](#openfoam-application-benchmark)
...
> \[!NOTE\] When the word **Input:** is mentioned, expect the next line to have commands that you need to copy and paste into your own terminal.
>
...
5.  Verify that the Docker Engine installation was successful by running the `hello-world` image
...
2.  Create and edit your monitoring configuration files

    ``` bash
    sudo nano /opt/monitoring_stack/docker-compose.yml
    ```

3.  Add the following to the `docker-compose.yml` YAML file
...
4.  Create and edit your Prometheus configuration files

    ``` bash
      sudo nano /opt/monitoring_stack/prometheus.yml
    ```

5.  Add the following to your `prometheus.yml` YAML file
...
6.  Configure your Prometheus data sources

    ``` bash
    sudo nano /opt/monitoring_stack/prometheus-datasource.yaml
    ```

7.  Add the following to your `prometheus-datasource.yaml`.
...
> \[!IMPORTANT\] The following is included to demonstrate the concept of TCP Port Forwarding. In the next section, you are: \* Opening a TCP Forwarding Port and listening on Port 3000 on your **workstation**, i.e. <http://localhost:3000> \* You are then binding this ***SOCKET*** to TCP Port 3000 on your **head node**.
>
...
> ```
>
> Make sure that you understand the above concepts, as it will facilitate your understanding of the following considerations: \* If you have successfully configured [WireGuard](../tutorial2/README.md#wirguard-vpn-cluster-access)
>
> ``` css
...
>
> Take the time now however, to ensure that all of your team members understand that there are a number of methods with which you can access remote services on your head node: \* <http://154.114.57.x:3000> \* <http://localhost:3000> \* <http://>`<headnode wireguard ip>`:3000 \* <http://>`<headnode zerotier ip>`:3000
...
2.  From a browser on your **workstation** navigate to the Grafana dashboard on your head node

3.  Go to a browser and login to Grafana:

    <figure>
    <img src="./resources/grafana_login.png" alt="Grafana Login." />
    <figcaption aria-hidden="true">Grafana Login.</figcaption>
    </figure>

4.  Login to your Grafana dashboards
...
<figure>
<img src="./resources/grafana_dashboards.png" alt="Grafana Login." />
<figcaption aria-hidden="true">Grafana Login.</figcaption>
</figure>
...
<figure>
<img src="./resources/grafana_new.png" alt="Grafana Login." />
<figcaption aria-hidden="true">Grafana Login.</figcaption>
</figure>
...
6.  Input: 1860 and click Load
    ...
    <figure>
    <img src="./resources/grafana_import.png" alt="Grafana Login." />
    <figcaption aria-hidden="true">Grafana Login.</figcaption>
    </figure>
...
<figure>
<img src="./resources/grafana_done.png" alt="Grafana Login." />
<figcaption aria-hidden="true">Grafana Login.</figcaption>
</figure>
...
- compute_node_ip:9100: The target address and port on the compute node where Node Exporter is running. <user@headnode>\_ip: The SSH connection details for the headnode.
...
2.  Open TCP port 8889 on your nftables firewall, and restart the service
...
> \[!TIP\] There are a number of plotting utilities available in Python. Each with their own advantages and disadvantages. You will be using [Plotly](https://plotly.com/python/ipython-notebook-tutorial/) in the following exercises.
...
2.  Install Project Jupyter and Plotly plotting utilities and dependencies
...
5.  Open a browser on your workstation and navigate to your JupyterLab server on your head node:

    ``` bash
    http://<headnode_public_ip>:8889
    ```

6.  Login to your JupyterLab server using your `<TOKEN>`.

7.  Create a new Python Notebook and plot your HPL results:
...
8.  Click on the camera icon to download and save your image. Post your results as a comment, replying to this [GitHub discussion thread](https://github.com/chpc-tech-eval/chpc24-scc-nmu/discussions/114).
...
3.  Append the following to your `qv_experiment.py` script:
...
4.  Run the benchmark by executing the script you’ve just written:
...
2.  Create a Terraform directory, descend into it and Edit the `providers.tf` file
...
3.  You must specify a [Terraform Provider](https://registry.terraform.io/browse/providers)
...
1.  Generate OpenStack API Credentials
    ...
    <figure>
    <img src="./resources/openstack_application_creds.png" alt="OpenStack Application Credentials." />
    <figcaption aria-hidden="true">OpenStack Application Credentials.</figcaption>
    </figure>

2.  Download and Copy the `clouds.yml` File
...
3.  Create `main.tf` Terraform File Inside your `terraform` folder, you must define a `main.tf` file. This file is used to identify the provider to be implemented as well as the compute resource configuration details of the instance we would like to launch.
...
1.  Generate and Deploy Terraform Plan Create a Terraform plan based on the current configuration. This plan will be used to implement changes to your Sebowa OpenStack cloud workspace, and can be reviewed before applying those changes. Generate a plan and write it to disk:
...
3.  Verify New Instance Successfully Created by Terraform Finally confirm that your new instance has been successfully created. On your Sebowa OpenStack workspace, navigate to `Project` → `Compute` → `Instances`.
...
1.  Create GitHub Repository If you haven’t already done so, sign up for a [GitHub Account](https://github.com/). Then create an empty private repository with a suitable name, i.e. `deploy_compute_node`:

    <figure>
    <img src="./resources/github_create_new_repo.png" alt="Github Create" />
    <figcaption aria-hidden="true">Github Create</figcaption>
    </figure>

2.  Add your team members to the repository to provide them with access: ![Github Manage Access](./resources/github_manage_access.png)

3.  If you haven’t already done so, add your SSH key to your GitHub account by following the instructions from [Steps to follow when editing existing content](../README.md#steps-to-follow-when-editing-existing-content).

> \[!TIP\] You will be using your head node to orchestrate and configure your infrastructure. Pay careful attention to ensure that you copy over your **head node**’s public SSH key. Administrating and managing your compute nodes in this manner requires you to think about them as “cattle” and not “pets”.
...
2.  Copy the `providers.tf` and `main.tf` files you had previously generated:
...
2.  Copy the following configuration into `.circle/config.yml`:
...
3.  `Init`ialize the Git Repository, `add` the files you’ve just created and `push` to GitHub: Following the instructions from the previous section where you created a new GitHub repo, execute the following commands from your head node, inside the `deploy_compute_node` folder:
...
Navigate to [CircleCI.com](https://circleci.com) to create an account, link and add a new GitHub project. 1. Create a new organization and give it a suitable name ![CircleCI](./resources/circleci_create_organization.png) 1. Once you’ve logged into your workspace, go to projects and create a new project ![CircleCI](./resources/circleci_create_project00.png) 1. Create a new IaC Project ![CircleCI](./resources/circleci_create_project01.png) 1. If your repository is on GitHub, create a corresponding project ![CircleCI](./resources/circleci_create_project02.png) 1. Pick a project name and a repository to associate it to ![CircleCI](./resources/circleci_create_project03.png) 1. Push the configuration to GitHub to trigger workflow ![CircleCI](./resources/circleci_successful_deploy.png)
...
<figure>
<img src="./resources/setting_up_secrets.png" alt="Creating Github Actions secretes" />
<figcaption aria-hidden="true">Creating Github Actions secretes</figcaption>
</figure>
...
4.  Create necessary directories: `bash  sudo mkdir -p /var/spool/slurm/d  sudo chown slurm:slurm /var/spool/slurm/d`
...
> \[!IMPORTANT\] Always source the bashrc FIRST, then set WM_NCOMPROCS. Setting WM_NCOMPROCS before sourcing will cause it to be overwritten.
...
OpenFOAM uses an unconventional approach to parallelisation via **domain decomposition**. Instead of running on a shared dataset, the mesh and fields are split into separate subdirectories — one per MPI rank:
...
> \[!NOTE\] Please be ready to present the `gromacs_log` files for the **1.5M_water** benchmark to the instructors.
